...
function Shortcuts() {
  ...
  const getFaviconUrl = async (url) => {
    try {
      ...
      // Try fetching HTML and extracting favicon
      const response = await fetch(domain, { mode: "no-cors" }); // will likely block for some sites, so fallback
      ...
      if (match) {
        ...
        if (hrefMatch && hrefMatch[1]) {
          let faviconLink = hrefMatch[1];
          if (!faviconLink.startsWith("http")) {
            faviconLink =
              domain +
              (faviconLink.startsWith("/") ? faviconLink : `/${faviconLink}`);
          }
          return faviconLink;
        }
      }
    ...
    // Fallback to Google's favicon API
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (currentShortcuts.name && currentShortcuts.url) {
      const faviconUrl = await getFaviconUrl(currentShortcuts.url);
      const newShortcut = {
        ...currentShortcuts,
        favicon: faviconUrl,
      };
                ...
                <div className="w-14 h-14 bg-slate-800/20 backdrop-blur-sm rounded-full border border-slate-700/20 shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-110 hover:border-blue-500/30 flex items-center justify-center mb-2 group-hover:shadow-blue-500/20 hover:bg-slate-700/30">
                  {shortcut.favicon ? (
                    <img
                      ...
                      className="w-8 h-8 rounded-lg"
                      onError={(e) => {
                        e.target.src = `https://www.google.com/s2/favicons?domain=${shortcut.url}&sz=64`;
                      }}
                    />
                  ) : (
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-700/50 p-8 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/30 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-slate-300 text-sm font-medium mb-2">
            ...
            <div className="flex gap-4">
              <button
                onClick={handleSubmit}
                disabled={!currentShortcuts.name || !currentShortcuts.url}
                className="flex-1 bg-gradient-to-r from-emerald-500 to-blue-600 text-white px-8 py-3 rounded-xl shadow-lg hover:scale-105 transition-all disabled:opacity-50"
              >
                Save Shortcut
              </button>
              <button
                onClick={() => setShowAddForm(false)}
                className="px-8 py-3 bg-slate-700/50 text-slate-300 hover:bg-slate-600/50 hover:text-white rounded-xl"
              >
                Cancel
              </button>
            </div>
